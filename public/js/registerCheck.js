...
const checkBirthDateFormat = (strVal) => {
  if (!strVal) throw `Error: You must supply a Birth Date!`;
  if (typeof strVal !== "string") throw `Error: Birth Date must be a string!`;
  strVal = strVal.trim();
  if (strVal.length === 0)
    throw `Error: Birth Date cannot be an empty string or string with just spaces`;
