  ...
  .get(async (req, res) => {
    try {
      ...
      if(req.session.user.role=="student"){
        res.render('modules/allModules', {moduleList: moduleList,student:true});
      } 
      if(req.session.user.role=="faculty"){
        res.render('modules/allModules', {moduleList: moduleList,faculty:true});
      }
      if(req.session.user.role=="admin"){
        res.render('modules/allModules', {moduleList: moduleList,admin:true});
      }
...
router
  ...
  .get(async (req, res) => {
    ...
    if(req.session.user){
      ...
      if(req.session.user.role=="feculty"){
        return res.render('modules/newModule');
      }
      // if(req.session.user.role=="admin"){
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      if (!moduleinfo.mod_title|| !moduleinfo.mod_description) throw 'All fields need to have valid values';
      moduleinfo.mod_title = validation.checkString(moduleinfo.mod_title, 'title');
      // anninfo.userId = validation.checkId(anninfo.userId, 'userId');
      // anninfo.description
    } catch (e) {
      return res.status(400).render('modules/allModules',{error: e});
    }
    ...
    try {
      ...
      const newModule = await modulesData.create(title, user, description, course);
      if(!newModule){
        throw 'Could not post module'
      }
      return res.redirect('/module');
    } catch (e) {
      res.status(400).render('modules/allmodules',{error: `${e}`});
    }
  });
...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      res.render('modules/moduleDetail',{ title: title,user:user ,description:desc, date:date});
    } catch (e) {
      res.status(404).render('modules/allModules',{error: `${e}`});
    }
  })
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      if(deletedMod){              
        res.render('modules/allModules', {modList: modList,faculty:true, message:"Deleted Module"});  
      }else{
        res.render('modules/allModules', {modList: modList,faculty:true, message:"Could not delete Module"});
      }
    } catch (e) {
      res.status(404).render('modules/allModules',{ error: `${e}`})
    }
